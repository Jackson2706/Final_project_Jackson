...
def draw_bboxes(image, bboxes, line_thickness=None):
    ...
    for i, bbox in enumerate(bboxes):
        try:
            track_id, x, y, w, h, label, conf, _ = bbox.get_infor()
            color = (0, 255, 0)
            x1 = int(x - w / 2)
            y1 = int(y - h / 2)
            x2 = int(x + w / 2)
            ...
            cv2.putText(image, ' ID {} - {}'.format(track_id, label), (c1[0], c1[1] - 2), 0, line_thickness / 3,
    ...
    def update(self, bboxes_traject, image):
        ...
        if len(bboxes) > 0:
            ...
            for center_x, center_y, w, h, track_id, conf, timestamp in list(outputs):
                ...
                bboxes2draw.append(DetectedObject(track_id=track_id,
                                                       x_center=center_x,
                                                       y_center=center_y,
                                                       width=w,
                                                       height=h,
